...
void Shader::loadShadersFromFile(const char* vsFilePath, const char* fsFilePath, const char* gsFilePath)
{
	...
	if (gsFilePath != nullptr)
	{
		...
		setupShaders(vsCode, fsCode, nullptr);
	}
}

bool Shader::setupShaders(const char * vsCode, const char * fsCode, const char* gsCode)
{
	...
	if(gsCode != nullptr) glAttachShader(ID, gShader);
	glLinkProgram(ID);
    checkLinkErrors();
    ...
    if(gsCode) glDeleteShader(gShader);
